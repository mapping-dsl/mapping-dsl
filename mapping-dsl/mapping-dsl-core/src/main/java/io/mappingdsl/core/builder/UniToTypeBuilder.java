...
public class UniToTypeBuilder<SRC_ROOT> {
    ...
    public <TRG_ROOT> UniSupplyExpressionBuilder<SRC_ROOT, TRG_ROOT> to(Class<TRG_ROOT> targetType) {
        MappingKey<SRC_ROOT, TRG_ROOT> mappingKey = new MappingKey<>(this.sourceType, targetType);
        return new UniSupplyExpressionBuilder<>(mappingKey, new MappingRules());
    }
